...
public:
    ...
    void on_message(Handler&& h) { message_handler_ = std::forward<Handler>(h); }

    template <typename Handler>
    void on_connect(Handler&& h) { on_connect_ = std::forward<Handler>(h); }
...
private:
    void do_connect() {
        ...
        resolver_.async_resolve(host_, std::to_string(port_),
            [this](std::error_code ec, asio::ip::tcp::resolver::results_type results){
                ...
                for (auto& r : results) {
                    if (r.endpoint().address().is_v4()) v4.push_back(r.endpoint());
                }
                ...
                if (!v4.empty()) {
                    ...
                    socket_.async_connect(v4.front(),
                        [this](std::error_code ec2){
                            if (ec2) { handle_disconnect(ec2); return; }
                            finish_connected();
                        });
                } else {
    ...
    void do_write_next() {
        if (write_queue_.empty() || !connected_) { writing_ = false; return; }
        writing_ = true;
